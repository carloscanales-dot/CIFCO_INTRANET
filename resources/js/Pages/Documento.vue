<template>
  ...
  <AuthenticatedLayout id="dashboard-page">
    ...
    <div class="py-6">
      <v-tabs v-model="tabModel" background-color="white" dark>
        ...
        <v-tab value="antisoborno" :class="tabModel === 'antisoborno' ? 'bg-white text-primary' : 'text-white'">Sistema de Gestión Antisoborno</v-tab>
      </v-tabs>
  ...
  <v-tabs-window-item value="antisoborno">
          <v-container>
            <!-- Outer panels por grupo (Antisoborno) -->
            <v-expansion-panels multiple>
              <v-expansion-panel v-for="grupo in gruposAntisoborno.filter(g => (categoriasPorGrupo[g.id] || []).length > 0)" :key="grupo.id">
                <v-expansion-panel-title class="font-bold">{{ grupo.nombre }}</v-expansion-panel-title>
                <v-expansion-panel-text>
                  <v-expansion-panels multiple>
                    <v-expansion-panel v-for="categoria in categoriasPorGrupo[grupo.id] || []" :key="categoria.id">
...
const gruposAntisoborno = computed(() => (props.grupos || []).filter(g => g.tipo === 'antisoborno'));
...
const verDocumento = (doc) => {
  ...
  if (url && url.startsWith('/')) {
    ...
    documentoSeleccionado.value = { ...doc, url: window.location.origin + encodedUrl };
  } else {
